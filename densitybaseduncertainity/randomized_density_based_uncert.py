# -*- coding: utf-8 -*-
"""
Created on Sat Feb 11 23:02:22 2023
@author: smash
"""
...
class RandomizedDensityBasedUncertainty:
    def __init__(self, df, thresh, model, Beta, Propotion, init_training):
        self.df = df
        self.thresh = thresh
        self.model = model
        self.Beta = Beta
    ...
    def create_randomized_density_based_uncertainty_accuracy_list(self):
        ...
        for x in range(len(z)):
            ...
            if x < self.initial_training:
                ...
                if (1 - abs(prob_0 - prob_1)) * dist ** (self.Beta) >= self.thresh or random.random() < self.Propotion:
                    self.model = self.model.learn_one(a, b)
                    training_data += 1
                    training_data_vec.append(a)
